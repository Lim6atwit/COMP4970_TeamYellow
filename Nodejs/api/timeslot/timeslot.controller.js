const {
  ...
  module.exports = {
    ...
    QueryTimeslotById: (req, res) => {
      ...
      QueryTimeslotById(id, (err, results) => {
        if (err) {
          console.log(err);
          return;
        }
        ...
        (err, data) => {
          if (err) {
            if (err.error === "not_found") {
              res.send({
                message: `Not found Timeslot with id ${req.params.id}.`
              });
  ...
  deleteTimeslot(req.params.id, (err, data) => {
    if (err) {
      if (err.error === "not_found") {
        res.send({
          message: `could not found Timeslot with id ${req.params.id}.`
        });
